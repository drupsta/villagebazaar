...
//if(isset($_POST['product_category']) && ($parent_id!= null) )
//{
//$qry_subcat ="SELECT name, c.category_id  as category_id FROM oc_category_description cd ,oc_category c where cd.`category_id`=c.`category_id` and parent_id=$parent_id and status=1";
//$rs = mysql_query($qry_subcat);
//$row=mysql_fetch_array($rs); 
 //if($row==null){$error_subcat= "No subcategory exist at present";}
 //}
...
if ($row[0]==NULL)
{
echo $sql="INSERT INTO oc_category SET parent_id =".$_SESSION['category_id1']." , `top` = 0, `column` = 0, sort_order = 0 , status = 1, date_modified = NOW(), date_added = NOW()";
//mysql_query($sql);
...
echo $sql1="INSERT INTO oc_category_path(category_id,path_id,level)values($category_id,".$_SESSION['category_id1'].",0)";
// mysql_query($sql1);
...
//$sql="SELECT  category_id FROM oc_category_description WHERE name='$cat_name' and parent_id=0";
...
$category_id = mysql_insert_id();
$_SESSION['category_id']=$category_id;
echo $sql="INSERT INTO oc_category_description SET category_id = $category_id,language_id=1,name='$subcat_name',description='$desc1'";
mysql_query($sql);
...
////////////////////
?>
<div class="warning"><?php echo "Please add subcategory first"; 
    $_SESSION['category_save_button']=0;
$_SESSION['subcategory_save_button']=1;
    ?></div>
<?php
...
//$sorder=$_POST['sort_order'];
//$status=$_POST['status'];
//echo $qry_subcat ="SELECT name, c.category_id  as category_id FROM oc_category_description cd ,oc_category c where cd.`category_id`=c.`category_id` and parent_id=".$_SESSION['category_id'];
//$rs = mysql_query($qry_subcat);
//$row=mysql_fetch_array($rs); 
$sql="SELECT  category_id FROM oc_category_description WHERE name='".$_POST['displayValue']."' and status=1";
$rs = mysql_query($sql);
...
if(isset($_POST['subcategory_save_button']))
{
$test = $_POST['displayValue'];
 if($test!=null) {
$qry_subcat ="SELECT name, c.category_id  as category_id FROM oc_category_description cd ,oc_category c where cd.`category_id`=c.`category_id` and parent_id=$parent_id and status=1";
$rs = mysql_query($qry_subcat);
$row=mysql_fetch_array($rs); 
...
<div id="content">
  ...
  <div class="box">
	...
	<td align="left"><input type="submit" name="save"  value="Save" class="button" /></td>
		...
		<?php } ?> <input type="submit" name="subcategory_save_button"  value="Back" class="button" /></div>
     </div>
...
<!--<span style="color:blue"> If you want to add new category please do not select "Existing Categories" but only view them.  Select the "Existing Categories" only if you want to add new subcategory. </span>-->
          <td><span class="required">*</span><?php echo "Enter New Category: "; ?></td>
          <td> 
