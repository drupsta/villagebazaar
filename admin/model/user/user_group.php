...
class ModelUserUserGroup extends Model {
	public function addUserGroup($data) {
		...
		//$this->db->query("INSERT INTO " . DB_PREFIX . "user_group SET name = '" . $this->db->escape($data['name']) . "', view = '" . //(isset($data['view']) ? serialize($data['view']) : '') . "'");
	}
	public function getviews($data) 
    {
 
     $name_log = substr("$data", -19); 
    $sql = "SELECT * FROM " . DB_PREFIX . "user WHERE username = '" . $name_log . "'";
	$query = $this->db->query($sql);
     if(isset($query->row['user_group_id'])){
	$id=$query->row['user_group_id'];
	$sql = "SELECT * FROM " . DB_PREFIX . "user_group WHERE user_group_id = '" . $id . "'";
	$query = $this->db->query($sql);
	//print_r($query);
	return $query->rows; 
  }
	...
	public function editUserGroup($user_group_id, $data) {
		...
		//$this->db->query("UPDATE " . DB_PREFIX . "user_group SET name = '" . $this->db->escape($data['name']) . "', view = '" . //(isset($data['permission']) ? serialize($data['permission']['view']) : '') . "' WHERE user_group_id = '" . (int)$user_group_id . "'");
	}
