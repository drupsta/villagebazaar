<?php
class ModelCatalogProductss extends Model {
		
	public function addProduct($data) {
$_SESSION['userid']=$this->user->getId();
				
	$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "',quantity = '" . (int)$data['quantity'] . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "',price = '" . (float)$data['price'] . "',status = '1', user_id = '" . $_SESSION['userid'] . "', date_added = NOW()");
	...
	public function editProduct($product_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "',quantity = '" . (int)$data['quantity'] . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "',price = '" . (float)$data['price'] . "',status = '1', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
...
$_SESSION['userid']=$this->user->getId();
		...
		if (!empty($data['filter_category'])) {
						$sql .= " INNER JOIN " . DB_PREFIX . "product_to_category pc1 ON (p.product_id=pc1.product_id AND pc1.category_id=".intVal($data['filter_category']).")";
						}
