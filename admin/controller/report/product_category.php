<?php
class ControllerReportProductCategory extends Controller { 
	public function index() {   
		$this->language->load('report/product_category');
...
//		}
		...
		if (isset($this->request->get['page'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('report/product_category', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);		
...
//		$this->data['filter_cec'] = $filter_cec;
		$this->data['sel_category'] = $sel_category;
                $this->data['product_category'] = $product_category;
		$this->template = 'report/product_category.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
