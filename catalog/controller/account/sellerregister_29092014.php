...
if (isset($this->error['username'])) {
		...
		if (isset($this->error['std'])) {
			$this->data['error_std'] = $this->error['std'];
		} else {
			$this->data['error_std'] = '';
		}
		...
		if (isset($this->request->post['std'])) {
    		$this->data['std'] = $this->request->post['std'];
		} else {
			$this->data['std'] = '';
		}
...
if ($this->model_account_seller->getTotalUsersByEmail($this->request->post['email'])) {
	...
	if ((utf8_strlen($this->request->post['telephone']) < 10) || (utf8_strlen($this->request->post['telephone']) > 10)
		...
		{
		if ((utf8_strlen($this->request->post['std']) < 3)
	...
	/*	$this->load->model('account/customer_group');
		...
		if ($this->request->post['csc_id'] == '') {
		 ...
		 if ((utf8_strlen($this->request->post['username']) < 3)|| !preg_match('/^[a-zA-Z0-9 ]*$/', $this->request->post['username'])  )
		 {
	...
	//	print_r($test1);
                if ($test1!=array()) 
		 {
      		$this->error['username'] = $this->language->get('error_username_unique');
    	}
 }		/* $this->load->model('account/seller');
