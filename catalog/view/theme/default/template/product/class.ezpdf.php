...
var $ez=array('fontSize'=>auto); // used for storing most of the page configuration parameters
...
function ezTable(&$data,$cols='',$title='',$options=''){
 ...
 //'colGap' => 5 ; //the space on the left and right sides of each cell
  ...
  $defaults = array(
    'shaded'=>1,'showLines'=>1,'shadeCol'=>array(0.9,0.9,0.9),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>8,'titleFontSize'=>10
    ,'titleGap'=>3,'lineCol'=>array(0,0,0),'gap'=>2,'xPos'=>'centre','xOrientation'=>'centre'
    ,'showHeadings'=>1,'textCol'=>array(0,0,0),'width'=>0,'maxWidth'=>0,'cols'=>array(),'minRowSpace'=>-100,'rowGap'=>2,'colGap'=>1
...
function ezImage($image,$pad = 5,$width =2,$resize = 'full',$just = 'top',$border = '1'){
	//beta ezimage function
	if (stristr($image,"h:\wamp\www\ISO\CDAC_Logo.jpeg"))//copy to temp file 
	{
		$fp = @fopen($image,"rb");
		while(!feof($fp))
   		{
