...
$a=$_SESSION['sel_category'];
$b=$_SESSION['product_category'];
...
$pdf->SetTitle('Products Based on category Report');
...
$query= "select name, price, date(date_added) as date_added from oc_product p, oc_product_description pd, oc_product_to_category pc where p.product_id=pd.product_id and pc.product_id=pd.product_id ";

		if (!empty($_SESSION['product_category'])) {
			$query .= " AND pc.category_id = '" . (int)$_SESSION['product_category'] . "'";
		}
	//echo $query;
