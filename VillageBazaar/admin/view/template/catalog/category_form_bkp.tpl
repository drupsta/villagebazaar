...
if (is_numeric($catid)){
$qry="SELECT name, description FROM `oc_category_description` cd,`oc_category` c where cd.`category_id`=c.`category_id` and  cd.`category_id`=$catid and `status`=1 ";
...
//mysql_connect("localhost","sanjaysaxena","sanj@y!@#");
...
$sql="SELECT oc_category.category_id FROM oc_category_description,oc_category  WHERE oc_category_description.category_id=oc_category.category_id and name='".$_POST['displayValue']."' and status=1 ";
$rs = mysql_query($sql);
$row=mysql_fetch_array($rs); 
if($row!=NULL)
{
...
<!--<span style="color:blue"> If you want to add new category please do not select "Existing Categories" but only view them.  Select the "Existing Categories" only if you want to add new subcategory. </span>-->
          <td><?php if($_SESSION['Category_Action']=="Insert Category"){echo "Enter New Category/ Select Category: ";?><span class="required"></span> </td>
          <td>
...
<!--<input align="left" type="submit" name="show_subcat" value="Show Subcategories" class="button" />-->
       ...
       <!--    </tr>
        ...
        <tr>
            <td>  <?php if(($_SESSION['Category_Action']=="Update Category") ) { echo "Edit Category Name:"; ?></td>
            
        <td>    <input name="displayValue3"   value="<?php  
			...
			{
			echo isset($_POST['displayValue3']) ? ucwords($_POST['displayValue3']) : '';
			}
 
 
 ?>" type="text" /> <?php if ($error_catname) { ?>
              <span class="error"><?php echo $error_catname; ?></span>
              <?php } }?>
            </td>
        </tr>
		<tr>		
		<td>
		 <?php if($_SESSION['Category_Action']!="Update Category" || (isset($_REQUEST['filter_category']))) { if($_SESSION['Category_Action']=="Insert Category")  { echo "Enter New Subcategory:";} else if (isset($_REQUEST['filter_category'])) {echo "Edit Subcategory Name:";}?> </td>
	<!--	<td> Enter New Subcategory:isset($_REQUEST['filter_category'])) </td>-->
...
<span class="error"><?php echo "error";}?></span></td>
         ...
         <tr>
<?php if($_SESSION['Category_Action']=="Insert Category") { ?>
         ...
         <!--    <td><textarea name="category_description[<?php echo $language['language_id']; ?>][description]" cols="80" rows="2"><?php echo isset($category_description[$language['language_id']]) ? $category_description[$language['language_id']]['description'] : ''; ?></textarea></td> -->
          ...
          <?php } ?>    </tr>
 <tr><?php if($_SESSION['Category_Action']=="Update Category") { ?>
                <td><?php echo $entry_description; ?></td>
