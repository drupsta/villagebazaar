...
///////
if ($_SESSION['Product_Action']=="Update Product") {
$productid=$_REQUEST['product_id'];
if (isset($productid)){
$qry= "SELECT `category_id` FROM `oc_product_to_category` WHERE `product_id`= $productid ";
$query= mysql_query($qry);
$res= mysql_fetch_array($query);
$sub_cat=$res['category_id'];

$qry1= "SELECT `name` FROM `oc_category_description` WHERE `category_id`=$sub_cat ";
$query1= mysql_query($qry1);
$res1= mysql_fetch_array($query1);
 $sub_catname= $res1['name'];
 
 $qry2= "SELECT `parent_id` FROM `oc_category` WHERE `category_id`=$sub_cat ";
 $query2= mysql_query($qry2);
$res2= mysql_fetch_array($query2);
 $main_cat= $res2['parent_id'];
 
 $qry3= "SELECT `name` FROM `oc_category_description` WHERE `category_id`=$main_cat ";
$query3= mysql_query($qry3);
$res3= mysql_fetch_array($query3);
 $main_catname= $res3['name'];
...
if ($_SESSION['Product_Action']=="Insert Product"){
...
if($price_duration_id!=NULL)
{
$qry="SELECT amount_percentage as tax FROM oc_pricing_policy where $price between from_price_range and to_price_range and status=1 and duration_policy_id=$price_duration_id";
...
<div id="content">
  ...
  <!--     <div id="tabs" class="htabs"><a href="#tab-general"> <?php echo $tab_general; ?></a>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form" >
		...
		?>
           ...
           <?php if ($error_category) { ?>
              ...
              <?php
						...
						echo "<option value='$main_cat' selected>$main_catname</option>";
						}		
						?>
            </select>
		...
		?>	
              ...
              <?php
						...
						echo "<option value='$sub_cat' selected>$sub_catname</option>";
						}		
						?>
            </select>
       </td>
	...
	<?php if ($_SESSION['Product_Action']=="Insert Product"){ ?>
                                      <tr>
...
foreach($curr_val as $cval=>$cval1)
		...
		<!--	 <tr>
          ...
          <?php } ?>
            ...
            <tbody id="image-row<?php echo $image_row; ?>">
              <tr>
			 <!--  <td class="left"><div id="1"><img src="<?php //echo $product_image['thumb']; ?>" alt="" id="thumb<?php echo $image_row; ?>" />-->
	...
	margin-right: 5px;
      ...
      <div class="button1">
      <?php  if ($_SESSION['Product_Action']=="Insert Product"){ ?>
          <a  onclick=pay(); align="center" class="button"><?php  echo "Pay" ; ?>
          
          </a><?php } else { ?>
          <a  onclick=save(); align="center" class="button"><?php  echo "Save" ; ?>
...
//    html += '    <td class="left"><div class="image"><img src="<?php //echo $no_image; ?>" alt="" id="thumb' + image_row + '" /><input type="hidden" name="product_image[' + image_row + '][image]" value="" id="image' + image_row + '" /><br /><a onclick="image_upload(\'image' + image_row + '\', \'thumb' + image_row + '\');"><?php echo $text_browse; ?></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a onclick="$('#thumb').attr('src', '<?php echo $no_image; ?>'); $('#image').attr('value', '');"><?php echo $text_clear; ?></a></div></td>';
...
function save(){
var a=confirm("Do you want to save the changes?");
if(a)
{
$('#form').submit();
}
...
{
alert("Changes have not been saved. Please click on Save button to save now else click on Back button to go to previous page");
