...
//		}
	...
	protected function validateDelete() {
		if (!$this->user->hasPermission('modify', 'catalog/category')) {
			$this->error['warning'] = $this->language->get('error_permission');
                ...
                foreach ($this->request->post['selected'] as $category_id) {
				$prod = $this->model_catalog_category->getCategoryProduct($category_id);
			}
                      //  print_r($prod);
        if($prod==array()){
            //echo "success";
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_category'])) {
			...
			$results = $this->model_catalog_category->getCat($data);
		...
		$output = '<option value="0">' . $this->language->get('text_add_new') . '</option>';
		
		$this->load->model('catalog/category');
		
		$results = $this->model_catalog_category->getSubcategory($this->request->post['parent_id']);
