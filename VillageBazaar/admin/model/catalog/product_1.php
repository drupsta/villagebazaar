...
$_SESSION['userid']=$this->user->getId();
	...
	$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "',producer = '" . $this->db->escape($data['producer']) . "',quantity = '" . (int)$data['quantity'] . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "',price = '" . (float)$data['price'] . "',price_duration_id = '" . (int)$data['price_duration_id'] . "',tax = '" . (float)$data['tax'] . "',status = '1', payment_status='Pending', user_id = '" . $_SESSION['userid'] . "', date_added = NOW()");
	...
	public function editProduct($product_id, $data) {
       // print_r($data);
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "',producer = '" . $this->db->escape($data['producer']) . "',quantity = '" . (int)$data['quantity'] . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "',price = '" . (float)$data['price'] . "',price_duration_id = '" . (int)$data['price_duration_id'] . "',tax = '" . (float)$data['tax'] . "',status = '0', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
...
$_SESSION['userid']=$this->user->getId();
	...
	//	$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)  LEFT JOIN " . DB_PREFIX . "order_status os ON (order_status_id = payment_status_id)";
	...
	$sql .= " WHERE   pd.language_id = '" . (int)$this->config->get('config_language_id') . "'"; 
		...
		//Added by Astha for Payment Status
                if (!empty($data['filter_payment_status'])) {
			$sql .= " AND p.payment_status LIKE '" . $this->db->escape($data['filter_payment_status']) . "%'";
		}
...
public function payProduct($product_id) {
		$query=$this->db->query("UPDATE " . DB_PREFIX . "product SET status = '1', payment_status ='Complete', date_available = NOW(), date_expiry = DATE_ADD(NOW(), INTERVAL (price_duration_id*30) DAY)  where product_id = '" . (int)$product_id . "'");
		//echo 'test';
